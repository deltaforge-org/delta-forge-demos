...
SELECT
    (SELECT COUNT(*) FROM {{zone_name}}.graph.entities)                                              AS total_entities,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.ratings)                                               AS total_ratings,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.entities WHERE entity_type = 'subscriber')             AS subscribers,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.entities WHERE entity_type = 'movie')                  AS total_movies,
    (SELECT COUNT(*) FROM {{zone_name}}.graph.ratings WHERE dst = 101)                               AS matrix_ratings;
