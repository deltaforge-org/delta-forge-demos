...
USING CSV LOCATION 'graph-karate-club-gpu/edges.csv'
...
LOCATION 'graph-karate-club-gpu/delta/edges'
AS SELECT
...
USING CSV LOCATION 'graph-karate-club-gpu/vertices.csv'
OPTIONS (header = 'true', delimiter = '|');


CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.karate_club.vertices
LOCATION 'graph-karate-club-gpu/delta/vertices'
AS SELECT
