...
USE {{zone_name}}.gpu_stress_network.gpu_stress_network
MATCH (a)-[r]->(b) ON GPU THRESHOLD 1
...
ASSERT ROW_COUNT = 25
USE {{zone_name}}.gpu_stress_network.gpu_stress_network
CALL algo.pageRank({dampingFactor: 0.85, iterations: 5}) ON GPU THRESHOLD 1
...
ASSERT ROW_COUNT = 25
USE {{zone_name}}.gpu_stress_network.gpu_stress_network
CALL algo.betweenness({samplingSize: 1000}) ON GPU THRESHOLD 1
...
ASSERT WARNING ROW_COUNT >= 2
USE {{zone_name}}.gpu_stress_network.gpu_stress_network
CALL algo.louvain({resolution: 1.5}) ON GPU THRESHOLD 1
YIELD node_id, community_id
RETURN community_id, count(*) AS size
ORDER BY size DESC
LIMIT 25;
...
-- least 1 node." THRESHOLD 1 holds for any non-empty graph, so these queries never fall back to CPU.
-- Results must be identical regardless of which device executes.


-- ============================================================================
-- 20. GPU THRESHOLD — Connected components, forced GPU
-- ============================================================================
-- THRESHOLD 1: the GPU executes for any non-empty graph.
...
-- ============================================================================
-- 21. GPU THRESHOLD — Connected components, forced GPU
-- ============================================================================
-- THRESHOLD 1 forces GPU execution; this never falls back to CPU.
...
-- ============================================================================
-- 22. GPU THRESHOLD — PageRank, forced GPU
-- ============================================================================
-- THRESHOLD 1: the GPU executes PageRank for any non-empty graph.
