...
SELECT COUNT(*) AS row_count FROM {{zone_name}}.political_books.vertices;

-- Verify edge count (441 undirected edges x 2)
ASSERT VALUE row_count = 882
SELECT COUNT(*) AS row_count FROM {{zone_name}}.political_books.edges;
...
FROM {{zone_name}}.political_books.edges e
WHERE NOT EXISTS (SELECT 1 FROM {{zone_name}}.political_books.vertices v WHERE v.vertex_id = e.src)
   OR NOT EXISTS (SELECT 1 FROM {{zone_name}}.political_books.vertices v WHERE v.vertex_id = e.dst);
...
FROM {{zone_name}}.political_books.edges
WHERE src = dst;
...
FROM (
    SELECT MAX(deg) AS max_deg FROM (
        SELECT src, COUNT(*) AS deg FROM {{zone_name}}.political_books.edges GROUP BY src
    )
)
...
FROM (
    SELECT MIN(vertex_id) AS min_id, MAX(vertex_id) AS max_id FROM {{zone_name}}.political_books.vertices
)
...
FROM (
    SELECT COUNT(*) AS cnt FROM {{zone_name}}.political_books.edges WHERE weight <> 1.0
)
...
FROM (
    SELECT COUNT(*) AS cnt FROM {{zone_name}}.political_books.edges e1
    WHERE NOT EXISTS (
        SELECT 1 FROM {{zone_name}}.political_books.edges e2
...
FROM (
    SELECT COUNT(DISTINCT edge_type) AS cnt FROM {{zone_name}}.political_books.edges
);
