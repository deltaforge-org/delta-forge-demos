...
-- ============================================================================
-- 1M-row pure-fixed-width baseline. SUM(rn) is the closed form
-- N*(N+1)/2 = 1_000_000 * 1_000_001 / 2 = 500_000_500_000.

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 1000000
ASSERT VALUE sum_rn = 500000500000
ASSERT VALUE min_rn = 1
ASSERT VALUE max_rn = 1000000
SELECT
    COUNT(*) AS n_rows,
    SUM(rn) AS sum_rn,
    MIN(rn) AS min_rn,
...
-- ============================================================================
-- Query 2: bench.fixed_narrow per-cell deterministic spot-check at rn=12345
-- ============================================================================
...
ASSERT ROW_COUNT = 1
ASSERT VALUE i64_a = 12345
ASSERT VALUE i64_b = 86415
ASSERT VALUE i64_c = 57
ASSERT VALUE f64_a = 12345.0
ASSERT VALUE f64_b = 6172.5
ASSERT VALUE f64_c = 3.45
...
-- ============================================================================
-- 100K rows, 60 fixed-width cols. Multiples of 2 in [1, 100K] = 50_000;
-- multiples of 3 in [1, 100K] = floor(100_000/3) = 33_333.

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 100000
ASSERT VALUE sum_rn = 5000050000
ASSERT VALUE n_b01_true = 50000
ASSERT VALUE n_b02_true = 33333
SELECT
    COUNT(*) AS n_rows,
    SUM(rn) AS sum_rn,
...
-- ============================================================================
-- 500K rows, NULL when rn % 10 IN (0,1,2): exactly 30% NULL = 150_000 rows.
-- 350_000 non-null rows. md5*6 = 192 chars; lpad to 20 chars.

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 500000
ASSERT VALUE n_short_null = 150000
ASSERT VALUE n_short_not_null = 350000
ASSERT VALUE n_long_a_null = 150000
SELECT
    COUNT(*) AS n_rows,
...
-- ============================================================================
-- 100K rows, NULL when rn % 20 = 0: exactly 5% NULL = 5_000 rows.
-- Each non-null cell is exactly 50 chars (lpad).

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 100000
ASSERT VALUE n_k01_null = 5000
ASSERT VALUE n_k01_not_null = 95000
ASSERT VALUE n_k40_null = 5000
SELECT
    COUNT(*) AS n_rows,
...
-- buf_len boundaries. SUM(rn) closed form = 50_005_000.

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 10000
ASSERT VALUE sum_rn = 50005000
...
-- b3 size = 32 * (1 + rn%1024) -> [32, 32768]  bytes
-- N=5_000. SUM(rn) closed form = 12_502_500.

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 5000
ASSERT VALUE sum_rn = 12502500
...
-- ============================================================================
-- 500K rows. SUM(rn) closed form = 125_000_250_000. d_d at rn=12345:
...
-- ============================================================================
-- 500K rows, 30 nullable cols. Each col populated only when rn%20=0, so
-- non-null = 25_000 and null = 475_000 per column. We sample columns of
-- different physical layouts (BIGINT, STRING, DECIMAL, BOOLEAN).

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 500000
ASSERT VALUE n_l01_not_null = 25000
ASSERT VALUE n_l01_null = 475000
ASSERT VALUE n_s01_not_null = 25000
ASSERT VALUE n_m01_not_null = 25000
ASSERT VALUE n_bo01_not_null = 25000
SELECT
    COUNT(*) AS n_rows,
...
-- ============================================================================
-- 500K rows. skew is 100_000 chars when rn%100=0 (5_000 rows), otherwise
-- CAST(rn AS STRING) which is 1..6 chars for rn in [1, 500_000].

ASSERT ROW_COUNT = 1
ASSERT VALUE n_rows = 500000
ASSERT VALUE n_long_skew = 5000
ASSERT VALUE n_short_skew = 495000
ASSERT VALUE max_skew_len = 100000
SELECT
    COUNT(*) AS n_rows,
    SUM(CASE WHEN LENGTH(skew) = 100000 THEN 1 ELSE 0 END) AS n_long_skew,
    SUM(CASE WHEN LENGTH(skew) <= 6     THEN 1 ELSE 0 END) AS n_short_skew,
...
-- closed-form SUM(rn). If any number drifts, the table name in that row
-- points to the regressing wire path.

ASSERT ROW_COUNT = 10
ASSERT RESULT SET INCLUDES
    ('fixed_narrow',     1000000, 500000500000),
    ('fixed_wide',        100000,   5000050000),
    ('string_narrow',     500000, 125000250000),
    ('string_wide_kv',    100000,   5000050000),
    ('string_long',        10000,     50005000),
    ('binary_blobs',        5000,     12502500),
    ('decimal_temporal',  500000, 125000250000),
    ('nested_json',        50000,   1250025000),
    ('null_heavy',        500000, 125000250000),
    ('skewed_strings',    500000, 125000250000)
