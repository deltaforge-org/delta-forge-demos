...
-- grid_readings_delta (UniForm) and grid_readings_iceberg_readback (external
-- iceberg view) are both created in setup.sql, along with the bulk INSERT
-- that populates the UniForm table. Queries below just read through the
-- iceberg view to verify UniForm shadow metadata is correct.
