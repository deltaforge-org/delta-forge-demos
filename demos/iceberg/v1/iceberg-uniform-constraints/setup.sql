...
-- --------------------------------------------------------------------------
-- Iceberg Read-Back View
-- --------------------------------------------------------------------------
-- Register an external Iceberg view over the same LOCATION as the Delta
-- UniForm table. UniForm wrote the iceberg metadata as a side effect of the
-- INSERT above, so this CREATE can run here. Putting it in setup.sql (not
-- queries.sql) means queries.sql stays pure SELECTs and cleanup.sql owns all
-- destruction.
