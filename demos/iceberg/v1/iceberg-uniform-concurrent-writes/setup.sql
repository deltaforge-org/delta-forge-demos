...
CREATE DELTA TABLE IF NOT EXISTS {{zone_name}}.iceberg_demos.beta_corrections (
    ...
    ingested_at     VARCHAR,
    batch_id        VARCHAR
) LOCATION 'iceberg-uniform-concurrent-writes/beta_corrections'
TBLPROPERTIES (
    'delta.universalFormat.enabledFormats' = 'iceberg',
    'delta.universalFormat.icebergVersion' = '2',
    'delta.columnMapping.mode' = 'id'
...
-- ------------------------------------------------------------
-- Iceberg Read-Back View(s)
-- ------------------------------------------------------------
-- Created here in setup.sql so queries.sql stays read-only.
-- queries.sql verifies engine behavior; cleanup.sql owns destruction.
