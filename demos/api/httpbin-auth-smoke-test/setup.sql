...
-- endpoints). The two bronze external tables are created in queries.sql
-- after their INVOKE calls, because CREATE EXTERNAL TABLE validates and
-- detects schema against the landed files, so the landing folder must
-- exist first. The two INVOKE calls, the run audit, the schema detection,
-- and the assertions all live in queries.sql so the user can see the
-- X-API-Key round-trip end to end from a single file.
-- ============================================================================
...
CREATE CONNECTION IF NOT EXISTS httpbin_smoke
    ...
    OPTIONS (
        ...
        base_path        = 'httpbin-auth-smoke-test/httpbin_smoke',
        timeout_secs     = '30'
    )
