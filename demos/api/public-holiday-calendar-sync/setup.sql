...
--
-- The bronze external table over the landed JSON is NOT created here. It
-- is created in queries.sql after the INVOKE, because CREATE EXTERNAL
-- TABLE validates and detects schema against the landed files, so the
-- landing folder must exist first.
--
-- The actual wave (next-year capture via SELECT ... INTO,
-- INVOKE ... USING, bronze table creation, schema detection, anti-join
-- merge) lives in queries.sql. It must run as a single multi-statement
-- script because
...
LOCATION 'public-holiday-calendar-sync/silver/country_holidays';
...
CREATE CONNECTION IF NOT EXISTS holiday_calendar
    ...
    OPTIONS (
        ...
        base_path    = 'public-holiday-calendar-sync/holiday_calendar',
        timeout_secs = '30'
    )
