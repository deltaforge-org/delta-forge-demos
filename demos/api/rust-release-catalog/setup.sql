...
-- Feature: REST API ingest catalog objects, connection, endpoint, and a
--          silver Delta table
-- ============================================================================
--
...
-- dashboards point at.
--
-- This file declares the catalog objects only: the zone, schema, REST API
-- connection, API endpoint, and the silver Delta table. The bronze
-- external table over the JSON landing is intentionally NOT created here.
-- CREATE EXTERNAL TABLE runs schema validation and detection against the
-- landed files, so the landing folder must exist first. That CREATE lives
-- in queries.sql and runs after the INVOKE has written its pages. The
-- INVOKE that issues the actual HTTPS GET, the run audit, the schema
-- detection, and the bronze->silver promotion all live in queries.sql so
-- the user can see the end-to-end ingest call from a single file.
-- ============================================================================
...
CREATE CONNECTION IF NOT EXISTS github_releases
    ...
    OPTIONS (
        ...
        base_path     = 'rust-release-catalog/github_releases',
        timeout_secs  = '30'
    );
...
LOCATION 'rust-release-catalog/silver/rust_releases';
