...
--
-- This file declares the connection, endpoints, and silver Delta table.
-- The bronze external table is NOT created here: CREATE EXTERNAL TABLE
-- validates and detects schema against the landed files, so it is
-- created in queries.sql after INVOKE has written the JSON pages. The
-- two INVOKE calls, the per-endpoint SHOW API ENDPOINT RUNS audit reads,
-- the bronze table creation, the schema detection, and the
-- bronze->silver promotion all live in queries.sql so the user can see
-- how the multi-endpoint connection is driven from SQL.
-- ============================================================================
...
CREATE CONNECTION IF NOT EXISTS frankfurter_fx
    ...
    OPTIONS (
        ...
        base_path    = 'frankfurter-fx-rate-catalog/frankfurter_fx',
        timeout_secs = '30'
    );
...
LOCATION 'frankfurter-fx-rate-catalog/silver/fx_rates';
