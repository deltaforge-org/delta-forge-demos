...
-- in a single landing folder. The historical backfill arrives as one large
-- JSON array (events_2026_q1.json); the daily increment arrives as
-- newline-delimited JSON (events_2026_apr.ndjson). Two physical shapes, one
-- logical feed.
--
...
-- not the file extension), recognises that .json (a top-level array) and
-- .ndjson (one object per line) both register USING JSON, unions their
-- discovered paths into one synthesized config, and registers the table.
--
